...
function buildPalette(seed: string, occasion: string) {
...
function frame(
  ...
  shadow = '0 6px 22px rgba(0,0,0,0.24),0 1px 5px rgba(0,0,0,0.12)'
): string {
  const ar  = orient === 'horizontal' ? '4/3' : '1/1';
  const rot = tilt !== 0 ? `transform:rotate(${tilt}deg);` : '';
  return (
    `<div style="overflow:hidden;border:${border};border-radius:3px;box-shadow:${shadow};${rot}${extraStyle}">` +
...
// ============================================================
const T = {
  h1: (t: string, px: number, col: string, shad: string) =>
    !t ? '' :
    `<div style="font-size:${px}px;font-weight:800;color:${col};letter-spacing:.03em;` +
    `line-height:1.15;font-family:Georgia,serif;text-shadow:${shad};">${t}</div>`,

  h2: (t: string, px: number, col: string) =>
    !t ? '' :
    `<div style="font-size:${px}px;font-weight:600;color:${col};letter-spacing:.05em;` +
    `margin-top:6px;font-family:Georgia,serif;">${t}</div>`,

  q: (t: string, px: number, col: string) =>
    !t ? '' :
    `<div style="font-size:${px}px;font-style:italic;font-weight:300;color:${col};` +
    `line-height:1.55;font-family:Georgia,serif;opacity:.88;">"${t}"</div>`,

  pill: (t: string, col: string) =>
    !t ? '' :
    `<span style="display:inline-block;border-radius:999px;padding:5px 22px;` +
    `border:1.5px solid ${col};font-size:17px;letter-spacing:.12em;` +
...
const grain =
  ...
  '<filter id="gf"><feTurbulence type="fractalNoise" baseFrequency=".68" numOctaves="4" ' +
  ...
  '<filter id="sf"><feGaussianBlur stdDeviation=".35"/></filter>' +
  '</defs></svg>';
...
function sparks(seed: string, col: string, n = 6): string {
  return Array.from({ length: n }, (_, i) => {
    ...
    return (
      `<div style="position:absolute;top:${top}%;left:${lft}%;width:${s}px;height:${s}px;` +
      `background:${col};${SHAPES[i % 2]};z-index:28;pointer-events:none;` +
      `filter:url(#sf);animation:sp ${dur}s ease-in-out ${del}s infinite alternate;opacity:.6;"></div>`
    );
  }).join('');
}
...
function buildMode_B_Prompt(p: {
  ...
  const style =
    ...
    '@keyframes sp{from{opacity:0;transform:scale(.3)rotate(0deg)}' +
    'to{opacity:.85;transform:scale(1)rotate(42deg)}}</style>';
  ...
  const grainOvl = '<div style="position:absolute;inset:0;z-index:1;pointer-events:none;' +
    'opacity:.09;filter:url(#gf);mix-blend-mode:overlay;"></div>';
  const sp = sparks(p.seed, pal.accentHex, 6);

  // ---- Text blocks (placed in safe open zones, NEVER top-center of any photo) ----
  const textTR =
    `<div style="position:absolute;top:${sz(4)}%;right:${sz(4)}%;z-index:22;` +
    `text-align:right;max-width:44%;pointer-events:none;">` +
    T.h1(p.occasionName, fs1, pal.textHex, pal.shadowCss) +
    T.h2(p.partnerName, fs2, pal.accentHex) +
    `</div>`;

  const textBL =
    `<div style="position:absolute;bottom:${sz(3)}%;left:${sz(4)}%;z-index:22;` +
    `max-width:58%;pointer-events:none;">` +
    T.pill(p.relationLabel, pal.accentHex) +
    `<div style="margin-top:6px;">${T.q(p.quote, fs3, pal.textHex)}</div>` +
    `</div>`;

  const textTC =
    `<div style="position:absolute;top:${sz(2)}%;left:0;width:100%;` +
    `text-align:center;z-index:22;padding:0 32px;pointer-events:none;">` +
    T.h1(p.occasionName, fs1, pal.textHex, pal.shadowCss) +
    T.h2(p.partnerName, fs2, pal.accentHex) +
    `<div style="margin-top:8px;">${T.q(p.quote, Math.round(fs3 * .9), pal.textHex)}</div>` +
    `</div>`;
  ...
  const t0 =
    style + grain + bg + grainOvl + textTR +
    `<div style="position:absolute;top:${sz(15)}%;left:${sz(3)}%;width:${sz(94)}%;` +
    `z-index:5;display:flex;flex-direction:column;align-items:center;gap:0;">` +
    frame(0, imgs[0]?.orientation ?? 'vertical', 'width:85%;', pal.tilts[0]) +
    (pc >= 2 ?
      `<div style="width:65%;margin-top:-${overlapPx}px;align-self:flex-end;` +
      `transform:rotate(${pal.tilts[1]}deg);">` + frame(1, imgs[1]?.orientation ?? 'vertical') + '</div>' : '') +
    (pc >= 3 ?
      `<div style="width:60%;margin-top:-${overlapPx}px;align-self:flex-start;` +
      `transform:rotate(${pal.tilts[2]}deg);">` + frame(2, imgs[2]?.orientation ?? 'vertical') + '</div>' : '') +
    (pc >= 4 ?
      `<div style="width:62%;margin-top:-${overlapPx}px;align-self:center;` +
      `transform:rotate(${pal.tilts[3]}deg);">` + frame(3, imgs[3]?.orientation ?? 'vertical') + '</div>' : '') +
    (pc >= 5 ?
      `<div style="width:56%;margin-top:-${Math.round(overlapPx * .75)}px;align-self:flex-end;` +
      `transform:rotate(${pal.tilts[4]}deg);">` + frame(4, imgs[4]?.orientation ?? 'vertical') + '</div>' : '') +
    '</div>' + textBL + sp;

  // Template 1: EDITORIAL GRID -- 2 top | text band | 1-3 bottom (gap: 0.75rem)
  const cols3 = Math.min(Math.max(pc - 2, 1), 3);
  const t1 =
    style + grain + grainOvl +
    `<div style="position:absolute;inset:0;display:grid;grid-template-rows:auto 1fr auto;z-index:5;">` +
    `<div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;padding:.75rem;">` +
    frame(0, imgs[0]?.orientation ?? 'vertical', '', 0, '3px solid #fff') +
    (pc >= 2 ? frame(1, imgs[1]?.orientation ?? 'vertical', '', 0, '3px solid #fff') : '<div></div>') +
    `</div>` +
    `<div style="background:${pal.bgHex};display:flex;flex-direction:column;` +
    `align-items:center;justify-content:center;padding:18px 32px;text-align:center;z-index:22;">` +
    T.h1(p.occasionName, fs1, pal.textHex, pal.shadowCss) +
    `<div style="margin-top:8px;">${T.h2(p.partnerName, fs2, pal.accentHex)}</div>` +
    `<div style="margin-top:6px;">${T.pill(p.relationLabel, pal.accentHex)}</div>` +
    `<div style="margin-top:8px;">${T.q(p.quote, fs3, pal.textHex)}</div>` +
    `</div>` +
    `<div style="background:${pal.bgHex};display:grid;` +
    `grid-template-columns:repeat(${cols3},1fr);gap:.75rem;padding:.75rem;">` +
    (pc >= 3 ? frame(2, imgs[2]?.orientation ?? 'vertical', '', 0, '3px solid #fff') : '') +
    (pc >= 4 ? frame(3, imgs[3]?.orientation ?? 'vertical', '', 0, '3px solid #fff') : '') +
    (pc >= 5 ? frame(4, imgs[4]?.orientation ?? 'vertical', '', 0, '3px solid #fff') : '') +
    `</div></div>` + sp;

  // Template 2: ASYMMETRIC MASONRY -- hero + 2x2 grid, translateY(-15%) overlap
  const t2 =
    style + grain + bg + grainOvl +
    ...
    textTC +
    `<div style="position:absolute;top:${sz(18)}%;left:${sz(4)}%;width:${sz(92)}%;z-index:5;">` +
    frame(0, imgs[0]?.orientation ?? 'vertical', '', 0, '3px solid #fff', '0 6px 24px rgba(0,0,0,0.22)') +
    `</div>` +
    `<div style="position:absolute;top:${sz(62)}%;left:${sz(4)}%;width:${sz(92)}%;` +
    `transform:translateY(-15%);z-index:6;">` +
    `<div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;">` +
    (pc >= 2 ? frame(1, imgs[1]?.orientation ?? 'vertical', '', 0, '3px solid #fff') : '<div></div>') +
    (pc >= 3 ? frame(2, imgs[2]?.orientation ?? 'vertical', '', 0, '3px solid #fff') : '<div></div>') +
    (pc >= 4 ? frame(3, imgs[3]?.orientation ?? 'vertical', '', 0, '3px solid #fff') : '<div></div>') +
    (pc >= 5 ? frame(4, imgs[4]?.orientation ?? 'vertical', '', 0, '3px solid #fff') : '<div></div>') +
    `</div></div>` +
    `<div style="position:absolute;bottom:${sz(3)}%;left:${sz(4)}%;width:${sz(92)}%;` +
    `text-align:center;z-index:22;pointer-events:none;">${T.q(p.quote, fs3, pal.textHex)}</div>` +
    sp;

  // Template 3: STAGGERED COLUMNS -- left col + right col offset 10% down
  const t3 =
    style + grain + bg + grainOvl + textTR +
    `<div style="position:absolute;top:${sz(14)}%;left:${sz(2)}%;width:${sz(96)}%;height:${sz(78)}%;` +
    `display:flex;flex-direction:row;gap:.75rem;z-index:5;padding:0 6px;">` +
    `<div style="flex:1;display:flex;flex-direction:column;gap:.75rem;">` +
    frame(0, imgs[0]?.orientation ?? 'vertical', '', pal.tilts[0], '12px solid #fff') +
    (pc >= 3 ? frame(2, imgs[2]?.orientation ?? 'vertical', '', pal.tilts[2], '12px solid #fff') : '') +
    `</div>` +
    `<div style="flex:1;display:flex;flex-direction:column;gap:.75rem;transform:translateY(10%);">` +
    (pc >= 2 ? frame(1, imgs[1]?.orientation ?? 'vertical', '', pal.tilts[1], '12px solid #fff') : '') +
    (pc >= 4 ? frame(3, imgs[3]?.orientation ?? 'vertical', '', pal.tilts[3], '12px solid #fff') : '') +
    (pc >= 5 ? frame(4, imgs[4]?.orientation ?? 'vertical', '', pal.tilts[4], '12px solid #fff') : '') +
    `</div></div>` + textBL + sp;

  const templates  = [t0, t1, t2, t3];
  const htmlBody   = templates[pal.layoutPick];
  const htmlTemplate =
    `<div style="width:1080px;height:1920px;aspect-ratio:9/16;position:relative;` +
...
export async function POST(req: Request) {
  try {
    ...
    const imageItems: { url: string; orientation: string }[] = rawPhotos
      .map((p: any) =>
        typeof p === 'string'
          ? { url: p, orientation: 'vertical' }
    ...
    console.log(
      `[Gemini][Styling] seed=${seed.slice(0, 8)} | ` +
      `occasion="${occasionName}" | photos=${imageItems.length} | model=${modelName}`
    );
