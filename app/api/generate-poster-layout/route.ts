...
// ============================================================
function aspectFromOrientation(orient: string): string {
...
function buildPalette(seed: string, occasion: string) {
  ...
  // [panelHue, panelSat, panelLum, accentHue, accentSat, accentLum]
  type Cfg = [number, number, number, number, number, number];
  const cfg: Cfg =
    occ.includes('jayanti') || occ.includes('gandhi')
      ? [38,  6, 96, 220, 42, 20]    // ivory + navy
    ...
    : occ.includes('birthday')
      ? [32, 10, 95,  25, 55, 26]    // warm cream + amber
    : occ.includes('wedding') || occ.includes('anniversary')
      ? [38, 8, 96, 345, 45, 26]     // champagne + rose
    : occ.includes('divas')
      ? [38, 6, 96, 30, 50, 22]      // cream + ochre (Aadivasi/Marathi Divas)
    : [(h % 360), 6, 92 + (h % 6), (h + 137) % 360, 38, 24] as Cfg;
  ...
  return { panelHex, textHex, accentHex, shadowCss, layoutPick };
}
...
// ============================================================
function sparks(seed: string, col: string): string {
  ...
  const sh     = [STAR, DIAMOND, STAR, DIAMOND];
  return Array.from({ length: 4 }, (_, i) => {
    ...
    return (
      ...
      `background:${col};${sh[i % 4]};z-index:25;pointer-events:none;` +
      `animation:sp ${dur}s ease-in-out ${del}s infinite alternate;opacity:.58;"></div>`
    );
  }).join('');
}
...
function buildMode_A_Prompt(p: {
  ...
  const split =
    '<!-- Photo: top 62% -- center center crop, face stays in frame center -->' +
    `<div style="position:absolute;top:0;left:0;width:100%;height:62%;overflow:hidden;z-index:2;">${photoEl}</div>` +
    `<div style="position:absolute;top:62%;left:0;width:100%;height:2px;background:${pal.accentHex}66;z-index:6;"></div>` +
    '<!-- Text panel: bottom 38% -- solid bg, professional gap 1.5rem+ via flex centering -->' +
    `<div style="position:absolute;top:62%;left:0;width:100%;height:38%;z-index:10;` +
    `display:flex;flex-direction:column;align-items:center;justify-content:center;` +
    `gap:1.5rem;padding:0 52px;text-align:center;background:${pal.panelHex};">` +
    headingEl + quoteEl + '</div>' +
    sparks(p.seed, pal.accentHex);
  ...
  const fade =
    `<div style="position:absolute;inset:0;overflow:hidden;z-index:2;">${photoEl}</div>` +
    '<!-- Strict smooth gradient: transparent 0-50%, rgba(0,0,0,0.9) at 100% -->' +
    `<div style="position:absolute;bottom:0;left:0;width:100%;height:55%;z-index:5;pointer-events:none;` +
    `background:linear-gradient(to bottom,rgba(0,0,0,0) 0%,rgba(0,0,0,0) 50%,rgba(0,0,0,0.9) 100%);"></div>` +
    '<!-- Text block: glass-morphism container for 100% legibility -->' +
    ...
    (p.quoteText
      ? `<div style="font-size:${fs2}px;font-style:italic;font-weight:300;` +
        ...
        `opacity:.92;text-shadow:0 1px 4px rgba(0,0,0,.70);">"${p.quoteText}"</div>`
      : '') +
    '</div>';
  ...
  const fullHtml =
    `<div style="width:1080px;aspect-ratio:${ar};position:relative;` +
    ...
    `opacity:.08;filter:url(#gf);mix-blend-mode:overlay;"></div>` +
    chosen +
    `</div>`;
...
${pal.layoutPick === 0
  ...
  - DO NOT apply any overlay on photo. Keep it clean and editorial.`
  : `Smooth Fade:
...
CONTRAST LAW:
${pal.layoutPick === 0
  ? `Text: ${pal.textHex} (dark). text-shadow: ${pal.panelHex} shifted -- never same color.`
  : `Text: near-white hsl(0,0%,96%). Shadow: rgba(0,0,0,0.88) -- very dark, high contrast.`}
...
export async function POST(req: Request) {
  try {
    ...
    const heroItem: { url: string; orientation: string } =
      !heroRaw ? { url: '', orientation: 'vertical' }
      : typeof heroRaw === 'string'
        ? { url: heroRaw, orientation: 'vertical' }
        : { url: heroRaw.url || '', orientation: heroRaw.orientation || 'vertical' };

    const hasPhoto = Boolean(heroItem.url);

    console.log(
      `[Gemini][Layout] seed=${seed.slice(0, 8)} | ` +
      `occasion="${occasionName}" | hasPhoto=${hasPhoto} | ` +
      `orientation=${heroItem.orientation} | model=${modelName}`
    );
